...
- The code base is flat (no sub-modules in `R/`). Function-level docs are roxygen2.
- **Never run `devtools::document()`.** Despite its `# Generated by roxygen2` header, `NAMESPACE` is maintained **by hand** — no roxygen block carries `@export`, so `document()` silently drops all 44 `export()` lines and breaks the package. Regenerate docs with `roxygen2::roxygenise(roclets = "rd")`, which only writes `man/` and leaves `NAMESPACE` alone. When adding an exported function, add its `export()` line to `NAMESPACE` by hand, in the block for its source file.
- `man/` is checked in and pkgdown needs it: every exported function must have a roxygen block *and* an entry in the `reference:` section of `_pkgdown.yml`; every internal function with a roxygen block must carry `@keywords internal`, or `pkgdown::check_pkgdown()` fails on "topics missing from index". Run that check before pushing doc changes.
