...
Version de tests : le cache de modèle d'inférence, introduit en 1.0.1, est
désormais couvert.

### Tests

* **Le cache de modèle d'inférence est testé, sans Python ni checkpoint** — le
  travail de performance de la 1.0.1 reposait sur une non-régression manuelle et
  jetable, qui exigeait un checkpoint de 520 Mo et des ortho locales. Or c'est la
  clé du cache qui décide si le checkpoint est relu, et elle se teste sans rien
  de tout cela : sa construction sort dans `.cle_modele_inference()`, fonction
  pure, et `reticulate` est mocké dans les tests — aucun interpréteur n'est
  démarré. Onze tests couvrent l'arrondi d'`img_size` au multiple de 32 supérieur
  (deux tuiles de 200 et 224 px partagent le même modèle plutôt que d'en faire
  reconstruire un à chaque fois), le fait que chaque composante de l'identité du
  modèle — checkpoint, architecture, canaux, `img_size` — change bien la clé, le
  non-rechargement au second appel identique, le rechargement forcé lorsque
  l'interpréteur Python a été relancé alors que le cache R croit encore détenir
  le modèle, et **qu'un chargement en échec remet la clé à `NULL`** : sans quoi
  l'appel suivant sauterait le chargement et `predict_tile()` partirait sur un
  `_OC_MODEL` inexistant.

La suite passe de 57 à 72 tests, la couverture de 7 % à 9 %.

# opencanopy 1.0.2

Version d'hygiène : `R CMD check` passe de six avertissements et trois NOTE à
`Status: OK`.

### Bug Fixes

* **Installer le paquet créait des répertoires dans le répertoire courant** —
  trois `dir_create()` au niveau supérieur de `R/` s'exécutaient au chargement
  du package, donc à l'installation, et créaient `data/`, `data/ign`,
  `data/open_canopy` et `outputs/` là où se trouvait le processus. D'où les deux
  avertissements `Subdirectory 'data' contains no data sets` et `Files not of a
  type allowed in a 'data' directory`. Les répertoires sont désormais créés par
  les fonctions qui écrivent dedans — `export_raster()`, `export_stats()`,
  `export_to_gpkg()`, `run_inference_python()`.
* **`compute_savi()` documentait un NDRE inexistant** — le bloc roxygen
  au-dessus de la fonction décrivait un « Normalized Difference Red Edge »
  approximé, avec sa propre formule et son `@return` : rien à voir avec le SAVI
  que la fonction calcule. Titre, description et paramètres réécrits, `L`
  documenté.

### Qualité du code

* **Caractères non-ASCII** — les 185 chaînes littérales accentuées passent en
  séquences `\uXXXX`. La sortie reste identique, en français ; commentaires et
  blocs roxygen gardent leurs accents, `R CMD check` ne vérifiant que le code
  analysé.
* **`library()` dans le code du paquet** — `library(reticulate)` disparaît au
  profit de `reticulate::` sur les 18 appels concernés, et le bloc de
  visualisation qualifie ses 43 appels en `ggplot2::`, `tidyterra::` et
  `patchwork::`. Les opérateurs `|` et `/` de patchwork continuent de
  fonctionner : `requireNamespace()` charge le namespace, ce qui suffit à
  enregistrer les méthodes S3.
* **Dépendances non déclarées** — `rprojroot` rejoint les `Suggests` et son
  unique appel est gardé par `requireNamespace()` ; les fonctions de base
  utilisées sans déclaration (`colorRampPalette`, `pdf`, `par`, `dev.off`,
  `cor`, `write.csv`, `download.file`…) sont importées explicitement dans
  `NAMESPACE`.
* **Documentation** — seize fonctions avaient un bloc roxygen réduit à son
  titre : leurs arguments sont documentés. Les `[0, 1]` et `[0, 255]` du
  roxygen, pris pour des liens Rd manquants, passent en code inline.

### Intégration continue

* Les actions GitHub passent aux versions ciblant Node 24 (`actions/checkout@v5`,
  `codecov/codecov-action@v5`) : GitHub les y forçait déjà en émettant un avis de
  dépréciation à chaque exécution. L'avis restant provient d'une dépendance
  interne de `codecov-action`.
* `.claude` rejoint `.Rbuildignore`.

# opencanopy 1.0.1

Version de performance et de remise en état de la chaîne d'intégration. Elle
solde le brief du 2026-09-14 (coût de l'inférence, OOM au calcul des indices)
et publie les correctifs de tuilage restés sans version depuis la 1.0.0.

### Performance

* **Le modèle était reconstruit à chaque tuile** — `predict_tile()` exécutait
  un script Python complet qui relisait le checkpoint (~520 Mo), reconstruisait
  PVTv2-b3 et rejouait le remapping des 568 clés, pour *chacune* des tuiles.
  Sur un AOI de 15 tuiles : 7,8 Go de checkpoint relus et 24,3 Go d'E/S mesurés
  sur le processus enfant. Le script est coupé en deux : `init_inference_model()`
  construit le modèle une fois et le laisse dans l'environnement `reticulate`
  persistant, `predict_tile()` ne fait plus que lire le raster, la passe avant
  et l'écriture. Un cache (checkpoint + architecture + canaux + `img_size`)
  évite toute reconstruction ; `run_inference()` fixe `img_size` sur la plus
  grande tuile pour que la clé reste stable sur toute la série.
* **Le backbone tournait deux fois par tuile** — un bloc de debug appelait
  `model.model.forward_features()` uniquement pour imprimer les statistiques
  d'activation, puis `model()` refaisait le même calcul ; le résultat du premier
  appel était jeté. Le backbone (attention spatiale du PVTv2) est la partie
  chère. Le diagnostic est conservé à l'identique mais calculé pendant l'unique
  passe avant. Mesuré sur une tuile 320x320 à 1,5 m, trois passages consécutifs :
  **67,1 / 65,0 / 60,9 s avant, 61,4 (chargement unique inclus) / 23,3 / 17,8 s
  après**, soit environ **3,1x** par tuile en régime établi. Non-régression
  vérifiée octet pour octet sur deux tuiles de tailles différentes.

### Bug Fixes

* **`terra` aveugle au plafond du cgroup — SIGKILL au calcul des indices** —
  sous `systemd-run --property=MemoryMax=12G`, `terra` dimensionne ses décisions
  « en mémoire vs sur disque » à partir de `/proc/meminfo`, qui n'est pas
  cgroup-aware : il voyait ~23 Go libres et se faisait tuer à 12 Go, après une
  heure, sur une ortho IRC de 233 millions de cellules — les deux CHM étaient
  déjà écrits, mais le run perdait une heure à chaque fois.
  `.limite_memoire_cgroup()` lit le plafond réel (cgroups v2 via
  `/proc/self/cgroup`, v1 en repli) en remontant toute la hiérarchie et en
  retenant le plus petit plafond ; `configurer_memoire_terra()` en donne la
  moitié à `terraOptions(memmax)`. Les quatre indices passent par
  `lapp(filename=)`, et les deux `resample()` du masque végétation écrivent
  également en fichier : le pic mémoire ne dépend plus de l'heuristique.
* **Le bloc de debug cassait le chemin UNet** — `smp.Unet` n'a pas d'attribut
  `.model`, donc `model.model.forward_features()` levait `AttributeError` et
  faisait échouer toutes les tuiles en architecture UNet. Le diagnostic est
  désormais conditionné à la présence de `forward_features`.
* **`pkgs_optional` non déclaré dans `test_pipeline_aoi.R`** — la déclaration
  était commentée alors que la boucle de vérification juste en dessous
  l'utilise : le script s'arrêtait sur `object 'pkgs_optional' not found` dès
  l'étape 1.

### Intégration continue et publication

* **Les badges pointaient vers un dépôt inexistant** — le dépôt a été renommé
  en `pobsteta/opencanopy` sans que rien ne suive : les quatre badges du README
  visaient `opencanopynemeton`, `_pkgdown.yml` publiait vers
  `pobsteta.github.io/opencanopynemeton/`, et `DESCRIPTION` portait une
  troisième orthographe, `open_canopy_nemeton`. Tout est aligné.
* **Le site pkgdown n'a jamais été publié** — `build_site_github_pages()`
  abandonnait sur la vérification d'URL (`URL` de `DESCRIPTION` ne contenait pas
  l'`url` de `_pkgdown.yml`), donc l'étape de déploiement n'était jamais
  atteinte et la branche `gh-pages` n'existait pas. L'index de référence
  échouait ensuite faute de fichiers `man/`, désormais générés.
* **Le job `coverage` échouait sur une dépendance manquante** —
  `covr::to_cobertura()` requiert `xml2`, un *Suggests* de covr que ni
  `extra-packages` ni `needs: coverage` ne tiraient. Le workflow R-CMD-check
  était rouge alors que le check et les tests passaient, et aucun rapport n'a
  jamais atteint Codecov.
* **Aucune Release n'était créée** — les tags `v0.1.2` et `v1.0.0` étaient
  poussés sans objet *Release*, la page du dépôt affichait donc encore `v0.1.1`.
  Un workflow `release.yml` publie désormais la Release au push d'un tag `v*`,
  avec la section `NEWS.md` correspondante et un garde-fou sur la cohérence
  entre le tag et `Version:` de `DESCRIPTION`.
* **`export_to_gpkg()` n'était pas documenté** et les fonctions internes du
  pipeline n'étaient pas marquées `@keywords internal`, ce qui bloquait l'index
  de référence pkgdown.

### Notes

* `NAMESPACE` est maintenu **à la main** malgré son en-tête « Generated by
  roxygen2 » : aucun bloc roxygen ne porte `@export`. `devtools::document()`
  supprime donc les 44 exports. Utiliser
